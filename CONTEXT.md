# C++ exam prep — chat context

Use this file as shared context when starting a new chat. Goal: avoid re-explaining the setup every time.

## Who / what

- **Student** preparing for a university **C++ / OOP** exam (course: *Програмни Езици*, prof. О. Наков).
- Source syllabus: `c++conspect.md`
- Workspace is mainly for notes, theory drafts, and small CLI practice programs.

## Exam format

| Part | What happens |
|------|----------------|
| **Practical** | **1 exercise** — write a **CLI program** in C++ (console app). Expect classes/objects, hierarchy, I/O, collections-style usage, etc. |
| **Theory** | **2–3 questions**. Typically: you get **one theme** from the conspect and must write **at least a solid paragraph**, ideally ~**half a page**. |

Theory answers should be clear, structured Bulgarian (or English if the chat asks), with definitions + short C++-oriented explanation and maybe a tiny example when it helps.

## Scope rules (important)

1. **In scope:** C++ and general OOP design from the conspect (see filtered list below).
2. **Out of scope — skip:**
   - Themes marked with **`*`** on the original conspect
   - **.NET / C#**-specific themes (interfaces in .NET, async/await as .NET, contracts in .NET, etc.)
3. When a theme mixes general OOP with .NET (e.g. generics, Liskov + .NET contracts, lambdas C++/C#): cover the **C++ / general OOP** part only.

## How to help in chats

- **Theory:** short essay-ready notes per theme (paragraph → half page), definitions, comparison tables, mini C++ snippets.
- **Practice:** CLI programs — readable structure, headers/sources if useful, compile with common `g++`/`clang++`.
- **Don’t** expand into full .NET stacks, ASP.NET, or starred “deep dive” contract topics unless explicitly asked later.
- Prefer **incremental study**: one theme or one practice task at a time unless asked for a plan/overview.

## Exam themes (filtered)

Numbering matches the original conspect. Skipped items are listed at the end.
